...
-- 테이블 삭제 순서!
-- customer table, orders table이 customer.custid를 기준으로 "관계"를 맺음
-- customer table의 존재하는 회원만 orders 테이블에 데이터를 추가할 수 있음
-- 만약에 orders 테이블이 있는데, customer 테이블을 삭제(drop)하면?
-- orders 테이블은 어떤 고객의 생일 정보를 알고 싶어도 customer 테이블 자체가 날라갔기 때문에 알 수 없음.
-- pk-fk로 연결된 테이블은 외래키가 설정된 테이블 (참조 테이블) 먼저 삭제
...
insert into customer values
    ...
    ('dfgh', '이재은', '미국 뉴욕', '01033331237', '2006-06-07');
...
-- 참고
select * from customer where custname like '%수'; -- 이름이 몇자든지 마지막 글자가 수이면 다 선택
